...
export default function Home() {
  ...
  const fetchNFTs = async () => {
    ...
    const api_key = "O6hGD0xGQgWNxVS2BXw3hD2NHpsTwM_n"
    ...
    <main className="flex flex-col items-center justify-center py-8 gap-y-3">
      <div className='flex flex-col w-full justify-center items-center gap-y-2'>
        ...
        <label className='text-gray-600'>
          <input onChange={(e) => {setFetchForCollection(e.target.checked)}} type="checkbox"  className="mr-2"></input>Fetch for collection
        </label>
        <button className='disabled:bg-slate-500 text-white bg-blue-400 px-4 py-2 mt-3 rounded-sm w-1/5' onClick={() => { if(fetchForCollection) {
